...
final class MetalBackedView: UIView {
    ...
    private func gameRect() -> CGRect {
        // Library launches set MADEIRA_SCREEN_W/H to the game's render size, so the
        // surface keeps that aspect (16:9, full-screen wide, or 4:3) instead of the
        // legacy fixed 4:3. Desktop mode keeps its old geometry: its windows are
        // placed by the Winios compositor, not by this rect.
        let (gw, gh) = logicalSize
    ...
    private func mapTouch(_ touch: UITouch) -> (Int32, Int32) {
        ...
        let r = gameRect()
        let (lw, lh) = logicalSize
        let x = Int32(min(max((p.x - r.minX) * lw / r.width, 0), lw - 1))
        let y = Int32(min(max((p.y - r.minY) * lh / r.height, 0), lh - 1))
        return (x, y)
    }

    /// The logical Windows screen the game sees (driver_ios.c reports the same size).
    private var logicalSize: (CGFloat, CGFloat) {
        guard !desktopMode else { return (1024, 768) }
        let w = envInt("MADEIRA_SCREEN_W", 1024), h = envInt("MADEIRA_SCREEN_H", 768)
        return (CGFloat(w > 0 ? w : 1024), CGFloat(h > 0 ? h : 768))
    }

    /// Re-run layout after a launch changed the logical screen size.
    static func refreshGameLayout() {
        guard let v = keyboardTarget else { return }
        v.setNeedsLayout()
        v.layoutIfNeeded()
    }
...
struct ContentView: View {
    ...
    /// Full sequence lives in WineLauncher.swift (shared with the game library).
    private func runWineFullSequence() {
        WineLauncher.shared.runFullSequence()
    }
