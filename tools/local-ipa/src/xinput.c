...
#ifdef MADEIRA_FREESTANDING
/* Built on Windows with plain clang (--target=x86_64-pc-windows-msvc) and no
 * Windows SDK or CRT: the handful of types and kernel32 imports used here are
 * declared locally. The x86-64 DLL runs under FEX like the game's own DLLs. */
#include <stdint.h>
#include <stddef.h>
typedef uint32_t DWORD; typedef uint16_t WORD; typedef uint8_t BYTE; typedef int16_t SHORT;
typedef int BOOL; typedef uint16_t WCHAR; typedef unsigned int UINT; typedef long LONG;
typedef uintptr_t ULONG_PTR; typedef void *LPVOID; typedef void *HINSTANCE;
...
__declspec(dllimport) BOOL __stdcall DisableThreadLibraryCalls(HINSTANCE inst);
static LONG InterlockedCompareExchange(LONG *dest, LONG exchange, LONG comparand)
{
    return __sync_val_compare_and_swap(dest, comparand, exchange);
}
static uint64_t _strtoui64(const char *s, char **end, int base)
{
    uint64_t v = 0;
    (void)end; (void)base;
...
/* The compiler may emit calls to these for struct zeroing. */
